...
- **1차 화면은 신고 처리 큐 중심으로 시작한다.** 지금 admin 모듈에 `Report` 도메인 뼈대가 있으므로 가장 작은 운영 단위로 시작할 수 있다.
- **대시보드는 2차로 붙인다.** 신고 처리, 로그인, 권한 관리가 안정된 뒤 홈 화면에서 지표와 처리 큐 요약을 보여준다.
...
| GET | `/` | 1차에서는 `/reports`로 redirect, 2차에서 홈 대시보드 제공 | 필요 |
...
목표는 운영자가 목록과 상세 페이지를 계속 오가지 않고 한 화면에서 판단하도록 만드는 것이다.
...
- `/login`에서 정상 관리자 계정으로 로그인하면 `/reports`로 이동한다.
