...
#### 🗺️ 지도 기반 게시물 조회 (`GET /map/location/post`)
위치 정보와 줌 레벨을 기반으로 주변 게시물을 조회합니다. 사진이 등록된 게시물만 반환됩니다.

**요청 파라미터**

| 파라미터 | 타입 | 설명 |
|---------|------|------|
| `latitude` | Double | 현재 지도 중심 위도 |
| `longitude` | Double | 현재 지도 중심 경도 |
| `zoom` | Int | 줌 레벨 (10 ~ 20) |
...
| `file_url` | String? | 대표 이미지 URL |
| `like_count` | Long | 좋아요 수 |
| `reply_count` | Long | 댓글 수 |

**정렬 알고리즘**

복합 점수 기반으로 정렬합니다. 점수가 높을수록 지도 마커 우선순위가 높습니다.

```
점수 = 본인 게시물 보너스 + 신선도 점수 + 참여도 점수

본인 게시물 보너스 : 내 글이면 +10000 (항상 최상단)
...
- 점수 계산 및 정렬을 DB 내에서 처리하여 앱단 반복문 없음
