...
superpowers(writing-plans / executing-plans / subagent-driven-development 등)를
비롯한 어떤 스킬이나 계획 문서가 **"태스크마다 커밋"** 을 지시하더라도 따르지 않는다.
CLAUDE.md 가 스킬·계획보다 우선한다.

- 구현 계획을 쓸 때 **태스크별 커밋 스텝을 넣지 않는다.** 커밋은 계획 전체가 끝난
  마지막에 한 번만 둔다.
- 이미 그런 스텝이 적힌 계획을 실행할 때는 커밋 스텝을 건너뛰고, 마지막에 모아서
  한 번 커밋한다.
- 태스크 사이에는 커밋 대신 `./gradlew build` / 관련 테스트로만 검증하고 넘어간다.
